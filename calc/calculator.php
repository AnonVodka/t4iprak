<?php
    ...
    class Calculator {
        ...
        private function handle_parentheses(array &$chars = null, $add = true) : void {
            ...
            // return if there are no parentheses in our array
            ...
            reset($chars);
            for ($a = current($chars); key($chars) !== null; $a = next($chars)) {
                
                if (!in_array("(", $chars) && !in_array(")", $chars))
                    break; // no more parentheses in our calculation


                // if $a is an opening parentheses
                // and we havent found a closing parentheses
                // we save its key
                if ($a === "(" && $cP === -1)
                    $oP = key($chars);

                // if $a is an closing parentheses
                // save its key
                else if ($a === ")")
                    $cP = key($chars);
        ...
        // TODO: recode this using current, next, reset
        private function handle_multiply_divide(array &$chars = null, $add = true) : void {
            ...
            while ($exponent != false) {
                ...
                // add to order of execution
                if ($add)
                    $this->oOE[] = join(" ", array($f, "**", $s, "=", $chars[$exponent]));
